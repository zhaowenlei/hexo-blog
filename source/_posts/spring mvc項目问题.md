...
遗憾的是出现 <font color=red size=20>color=Unable to compile class for JSP</font> 的错误.
　　网上寻找资料后知晓, 该问题由maven引入的servlet-api和tomcat自带的servlet-api版本发生了冲突.
　　解决方案如下:
　　maven的pom文件中, 把<font color=red size=20>color=servlet-api</font>依赖项的<font color=red size=20>color=scope</font>改为<font color=red size=20>color=provided</font>(即参与compile和test, 但不参与package).
